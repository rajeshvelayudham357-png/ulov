...
import {
  DEFAULT_MALE_ACHIEVEMENT_BADGES,
...
export const ensureMaleAchievementSchema = async () => {
  if (schemaReady) {
    return;
  }
  ...
  await sequelize.query(
    `CREATE TABLE IF NOT EXISTS male_achievement_badges (
  ...
  for (const badge of DEFAULT_MALE_ACHIEVEMENT_BADGES) {
    await sequelize.query(
      `INSERT IGNORE INTO male_achievement_badges
